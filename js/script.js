    ...
    onShow() {
        ...
        data.forEach(({date:{name, address, id, email, phone}, editMode}) => {
            list += `<li class="contact__item">
                            ...
                            ${editMode ? "<button data-save="+id+">Save</button>" : '<button class="contact__item__edit" data-edit='+id+'>Редактировать</button></div>'}       
                    </li>`
        })
...
window.addEventListener('load', () => {
     document.cookie = 'contacts=storageExpiration; max-age=(10*24*60*60))'

    //document.cookie = 'contacts=storageExpiration; max-age=dateNow.setDate(28)';
